...
class BuiltInFunction(BaseFunction):
    ...
    def execute_input(self, exec_ctx):
        text = input(exec_ctx.symbol_table.get("text").value)
    ...
    def execute_factorial(self, exec_ctx):
        return RTResult().success(Number(math.factorial(exec_ctx.symbol_table.get("x").value)))

    execute_factorial.arg_names = ["x"]

    def execute_random_int(self, exec_ctx):
        return RTResult().success(Number(random.randint(exec_ctx.symbol_table.get("x").value, exec_ctx.symbol_table.get("y").value)))

    execute_random_int.arg_names = ["x", "y"]

    def execute_random_string(self, exec_ctx):
        return RTResult().success(String(''.join(random.choice(string.ascii_letters + string.digits) for _ in range(exec_ctx.symbol_table.get('size').value))))

    execute_random_string.arg_names = ['size']

    def execute_cbrt(self, exec_ctx):
        return RTResult().success(Number(exec_ctx.symbol_table.get("x").value ** (1. / 3)))

    execute_cbrt.arg_names = ["x"]

    def execute_degrees(self, exec_ctx):
        return RTResult().success(Number(math.degrees(exec_ctx.symbol_table.get("x").value)))

    execute_degrees.arg_names = ["x"]

    def execute_stop(self, exec_ctx):
        quit(exec_ctx.symbol_table.get("code").value)

    execute_stop.arg_names = ["code"]
    ...
    def execute_run(self, exec_ctx):
        ...
        fn = fn.value
        fn = fn.split(".")
        if not fn[1] == 'ps' or fn[1] == 'pp':
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                "File must be parse script (.ps) or parse package (.pp)",
                exec_ctx
            ))

        if fn[1] == 'ps':
            try:
                with open(f"{fn[0]}.ps", "r") as f:
                    script = f.read()
            ...
            try:
                with open(f"{fn[0]}.pp", "r") as f:
                    script = f.read()
            except Exception as e:
                return RTResult().failure(RTError(
                    self.pos_start, self.pos_end,
                    f"Failed to load package \"{fn}\"\n" + str(e),
                    exec_ctx
                ))
            ...
            if error:
                return RTResult().failure(RTError(
                    self.pos_start, self.pos_end,
                    f"Failed to finish executing package \"{fn}\"\n" +
                    error.as_string(),
                    exec_ctx
                ))
...
BuiltInFunction.fact = BuiltInFunction("factorial")
BuiltInFunction.random_int = BuiltInFunction("random_int")
BuiltInFunction.random_string = BuiltInFunction("random_string")
...
BuiltInFunction.impor = BuiltInFunction("import")
...
global_symbol_table.set("is_str", BuiltInFunction.is_list)
...
global_symbol_table.set("random_str", BuiltInFunction.random_string)
...
global_symbol_table.set("define", BuiltInFunction.impor)
